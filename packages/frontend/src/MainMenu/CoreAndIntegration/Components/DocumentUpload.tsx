...
const DocumentUpload: React.FC = () => {
  ...
  const fetchContracts = async (customerId: string) => {
    try {
      const response = await axios.get(`http://localhost:3001/api/contract/customer/${customerId}`, {
        withCredentials: true,
        timeout: 5000
      });
    ...
    if (fileType === 'חוזה' && selectedCustomer) {
      for (const file of files) {
        try {
          ...
          const response = await axios.post(`http://localhost:3001/api/contract/customer/${selectedCustomer}/document`, {
