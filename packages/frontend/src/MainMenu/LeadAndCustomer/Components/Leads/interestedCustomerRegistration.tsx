...
const schema = z.object({
  ...
  phone: z
    ...
    .refine((val) => /^0\d{8,9}$/.test(val), { message: "מספר טלפון לא תקין" }),
  email: z.string().email("Invalid email").nonempty("חובה למלא אימייל"),
  idNumber: z
    .string()
    .nonempty('חובה למלא ת"ז')
  ...
  workspaceType: z
    .nativeEnum(WorkspaceType)
    .refine((val) => !!val, { message: "חובה למלא סוג חלל עבודה" }),
...
export const InterestedCustomerRegistration: React.FC = () => {
      ...
      content: (
        <>
          <InputField
            ...
            label="תאריך חתימת חוזה"
            required
            type="date"
          />
          <InputField
            name="contractStartDate"
            label="תאריך תחילת חוזה"
            required
            type="date"
          />
          <InputField
            name="billingStartDate"
            label="תאריך תחילת חיוב"
            required
            type="date"
          />
          <FileInputField
  ...
  return (
    <div className="interestedCustomerRegistration">
      ...
      {showForm ? (
        <div>
          ...
          <h4 className="text-lg text-center text-gray-600 my-2">
        ...
        <div className="text-center my-4">
          ...
          <Button
            onClick={() => navigate(`/leadAndCustomer/customers`)}
            variant="primary"
            size="sm"
          >
            למעבר לרשימת הלקוחות
          </Button>
        </div>
      )}
    </div>
