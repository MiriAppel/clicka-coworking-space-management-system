...
export const ContractDetails = () => {
  ...
  const { contracts, fetchContractsByCustomerId, loading, error } = useContractStore();
  ...
  useEffect(() => {
    ...
    fetchContractsByCustomerId(customerId)
      .then(() => {
        const result = useContractStore.getState().contracts.find(c => c.customerId === customerId) || null;
        setContract(result);
      })
    ...
    <div className="p-6 max-w-4xl mx-auto" dir="rtl">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button
          variant="primary"
          size="sm"
          onClick={() => navigate(`/leadAndCustomer/contracts/edit/${contract.id}`, {
            state: { customerName }
          })}
        >
          ערוך
