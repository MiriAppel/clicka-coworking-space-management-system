...
// //         // עדכון הסטייט לאחר מחיקת האינטראקציה
// //         set((state) => {
// //             const selectedLead = state.selectedLead;
// //             if (!selectedLead) {
// //                 return state; // אם אין selectedLead, מחזירים את הסטייט הקודם
// //             }
            
// //             const updatedInteractions = selectedLead.interactions.filter(interaction => interaction.id !== interactionId);
 ...
 handleDeleteLead: async (leadId: UUIDTypes) => {
  ...
  handleDeleteInteraction: async (interactionId: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/leads/${selectedLead.id}/interactions/${interactionId}`,
        {
          method: "DELETE",
        }
      );
  ...
  handleCreateInteraction: async (lead: Lead) => {
    try {
      console.log(lead);
      const response = await fetch(
        `http://localhost:3001/api/leads/${lead.id}/addInteraction`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
